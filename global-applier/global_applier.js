/**
 * 🕶️ Zero-Footprint PRO: Multi-Country 1-by-1 Sequential In-Browser Auto-Applier
 * 
 * GitHub: https://github.com/Naman-mahi/zero-footprint
 * CDN: https://cdn.jsdelivr.net/gh/Naman-mahi/zero-footprint@master/global-applier/global_applier.js
 * 
 * FEATURES:
 * - ⚡ 1-by-1 Sequential Processing: Never overloads CPU/RAM with hundreds of open tabs.
 * - 🎯 Configurable Batch Size (50 vs 100): Easily run 50 or 100 applications at a time with 1-click.
 ...
 * - ⏩ Batch Navigator: Next/Prev batch controls across all 974 global jobs (US, UK, IN, Remote).
...
(function () {
  ...
  if (window.__GLOBAL_APPLIER_INSTANCE__) {
    console.log("%c🔄 Cleaning up previous Global Applier session...", "color: #2563eb; font-weight: bold;");
    try {
      window.__GLOBAL_APPLIER_INSTANCE__.cleanup();
    } catch (e) {}
  }
  ...
  let batchSize = 50; // default 50, toggleable to 100

  // =========================================================================
  // 💾 STATE MANAGEMENT & LOCAL STORAGE PERSISTENCE
  // =========================================================================
  const STORAGE_KEY = "__GLOBAL_ZERO_FOOTPRINT_STATE__";
  ...
  const oldGlobalHud = document.getElementById("zero-footprint-global-hud");
  if (oldGlobalHud) oldGlobalHud.remove();
  ...
  hud.id = "zero-footprint-global-hud";
  hud.innerHTML = `
    <div style="
      position: fixed;
      top: 20px;
    ...
    ">
        ...
        <div style="display: flex; align-items: center; gap: 9px;">
          ...
          <div>
            <div style="display: flex; align-items: center; gap: 4px;">
              ...
              <span style="background: #eff6ff; color: #2563eb; font-size: 9.5px; font-weight: 700; padding: 1px 5px; border-radius: 5px; border: 1px solid #dbeafe;">GLOBAL</span>
            </div>
            <div style="font-size: 10.5px; color: #64748b; font-weight: 500;">Multi-Country 1-by-1 Job Applier</div>
          </div>
        </div>
        <div style="display: flex; align-items: center; gap: 4px;">
      ...
      <div id="zfp-body">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 6px 10px;">
          <div style="display: flex; align-items: center; gap: 4px;">
            <span style="font-size: 10px; color: #64748b; font-weight: 600;">Batch:</span>
            <button id="zfp-size-50-btn" class="zfp-size-btn" data-size="50" style="background: #2563eb; color: #ffffff; border: 1px solid #2563eb; border-radius: 5px; padding: 2px 6px; font-size: 10px; cursor: pointer; font-weight: 700;">50</button>
        ...
        <div id="zfp-log" style="
          ...
          border: 1px solid #e2e8f0;
          margin-bottom: 8px;
        ">Ready. Loaded ${jobQueue.length} jobs in queue. Select batch size (50 or 100) and click Start.</div>
        ...
        ">
          <div style="display: flex; align-items: center; gap: 4px; font-weight: 700; color: #1e293b; margin-bottom: 2px;">
            ${ICONS.info} 50 / 100 Batch Automation Guide:
          </div>
          <div>• <b>Batch Size:</b> Choose <b>50</b> or <b>100</b> applications per run.</div>
          <div>• <b>1-by-1 Flow:</b> Opens tab, clicks Apply trigger, and closes cleanly.</div>
          <div>• <b>Auto-Purge:</b> Clears cookies and storage between batches!</div>
        </div>
      </div>
    </div>
  `;
  ...
  function log(msg, color = "#2563eb") {
    console.log("%c[GlobalApplier] " + msg, "color: " + color + "; font-weight: 600;");
    if (logEl) {
      logEl.innerText = msg;
  ...
  async function processNextJob() {
    ...
    // Check if reached milestone (end of a batch of 50 or 100)
    if (state.currentIndex > 0 && state.currentIndex % batchSize === 0 && !state._milestonePassed) {
      ...
      log("🎉 Batch " + curBatch + " completed (" + batchSize + " jobs)! Storage purged. ⏳ Autostarting Batch " + nextBatch + " / " + totalBatches + " in 60s (or click button to start now)...", "#059669");
  ...
  // Batch Size Switcher (50 vs 100)
  const size50Btn = document.getElementById("zfp-size-50-btn");
  const size100Btn = document.getElementById("zfp-size-100-btn");

  size50Btn.addEventListener("click", () => {
    batchSize = 50;
    size50Btn.style.background = "#2563eb";
    size50Btn.style.color = "#ffffff";
    size50Btn.style.borderColor = "#2563eb";
    size50Btn.style.fontWeight = "700";

    size100Btn.style.background = "#ffffff";
    size100Btn.style.color = "#475569";
    size100Btn.style.borderColor = "#cbd5e1";
    size100Btn.style.fontWeight = "500";
    updateUI();
    log("Batch size set to 50 jobs per run.", "#2563eb");
  });

  size100Btn.addEventListener("click", () => {
    batchSize = 100;
    size100Btn.style.background = "#2563eb";
    size100Btn.style.color = "#ffffff";
    size100Btn.style.borderColor = "#2563eb";
    size100Btn.style.fontWeight = "700";

    size50Btn.style.background = "#ffffff";
    size50Btn.style.color = "#475569";
    size50Btn.style.borderColor = "#cbd5e1";
    size50Btn.style.fontWeight = "500";
    updateUI();
    log("Batch size set to 100 jobs per run.", "#2563eb");
  });
  ...
  function cleanupInstance() {
    ...
    isPaused = true;
    hud.remove();
    window.__GLOBAL_APPLIER_INSTANCE__ = null;
    delete window.__GLOBAL_APPLIER__;
    console.log("%c🧹 Global Applier session closed.", "color: #64748b; font-style: italic;");
  }
  ...
  const api = {
    ...
    setBatchSize: (size) => {
      if (size === 50 || size === 100) {
        batchSize = size;
        updateUI();
        log("Batch size set to " + size + " via API.", "#2563eb");
      }
    },
    wipeStorage: () => wipeAllStorageAndCookies(false),
    cleanup: cleanupInstance,
    getState: () => ({ ...state, batchSize }),
    setQueue: (urls) => {
  ...
  window.__GLOBAL_APPLIER_INSTANCE__ = api;
  window.__GLOBAL_APPLIER__ = api;

  console.log(
    "%c 🕶️ ZERO-FOOTPRINT: MULTI-COUNTRY 1-BY-1 AUTO-APPLIER %c READY ",
  ...
  console.log(
    ...
    "%c🎯 Batch Mode: %c" + batchSize + " applications per batch (Toggleable to 50 or 100)\n" +
    "%c💾 Saved Progress: %cJob " + (state.currentIndex + 1) + " (Batch " + (Math.floor(state.currentIndex / batchSize) + 1) + ") | Applied: " + state.completedCount + "\n" +
    "%c⏩ Batch Controls: %cClick 'Next Batch ▶' on HUD to jump batches\n" +
    "%c🧹 Storage Purge: %cClick 'Wipe Storage' to purge cookies & storage instantly\n" +
    "%c💡 Instructions: Click 'Start Batch " + (Math.floor(state.currentIndex / batchSize) + 1) + "' on HUD or call window.__GLOBAL_APPLIER__.start()",
