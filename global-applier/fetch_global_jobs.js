...
// Usage:
// node fetch_global_jobs.js <offset> <limit> [country]
//
// Examples:
// node fetch_global_jobs.js 0 500 ALL    (Global / Worldwide)
// node fetch_global_jobs.js 0 500 US     (United States)
// node fetch_global_jobs.js 0 500 GB     (United Kingdom)
// node fetch_global_jobs.js 0 500 CA     (Canada)
// node fetch_global_jobs.js 0 500 IN     (India)

const offset = Number(process.argv[2] || 0);
const requestedLimit = Number(process.argv[3] || 500);
const countryArg = (process.argv[4] || "ALL").toUpperCase();
const COUNTRY = countryArg === "ALL" ? null : countryArg;
...
function getOutputFilename() {
  ...
  const cLabel = COUNTRY ? `_${COUNTRY.toLowerCase()}` : "_global";

  return `out_${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}_${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}${cLabel}.json`;
}
...
async function fetchJobs() {
  ...
  while (jobUrls.length < totalToFetch) {
    ...
    console.log(
      `Fetching: offset=${currentOffset}, limit=${limit}, country=${COUNTRY || 'GLOBAL/ALL'}`
    );
...
(async () => {
  try {
    ...
    console.log(`Country Filter: ${COUNTRY || 'GLOBAL (ALL COUNTRIES)'}`);
